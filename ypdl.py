...
# AVOID MAKING STATIC METHODS. USE FUNCTIONS INSTEAD.
...
class YoutubePosts:
    ...
    async def scrape(self, pbar_position: int) -> None:
        ...
        with tqdm(total=len(self.posts), position=pbar_position, desc=f'{self.channel_name}', disable=None) as pbar:
            pbar.update(len(self.posts))
...
if __name__ == "__main__":
    # TODO: Change to Rich Progress bars
    ...
    yt_links = args['links']
    # yt_links = ['https://www.youtube.com/@theschooloflifetv']

    objects = [YoutubePosts(link, cookies) for link in yt_links]

    tasks = [obj.scrape(i) for i, obj in enumerate(objects)]
